...
namespace Unity.Cinemachine
{
    ...
    public class CinemachineGroupFraming : CinemachineExtension
    {
            ...
            VcamExtraState extra, ref CameraState state, float deltaTime)
        {
            ...
            var stateRot = state.GetCorrectedtOrientation();
            ...
            VcamExtraState extra, ref CameraState state, float deltaTime)
        {
            ...
            state.tOrientationCorrection = state.tOrientationCorrection * Quaternion.identity.ApplyCameraRotation(extra.RotAdjustment, up);
            stateRot = state.GetCorrectedtOrientation();
            ...
            if (Mathf.Abs(CenterOffset.x) > 0.01f ||Mathf.Abs(CenterOffset.y) > 0.01f)
            {
                ...
                else
                {
                    ...
                    state.tOrientationCorrection *= Quaternion.identity.ApplyCameraRotation(rot, state.ReferenceUp);
                }
            }
        }
