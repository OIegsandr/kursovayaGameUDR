...
namespace Unity.Cinemachine
{
    ...
    /// point to the center of the screen, based on the Follow target's tOrientation,
    ...
    public class CinemachineThirdPersonAim : CinemachineExtension
    {
            ...
            switch (stage)
            {
                ...
                case CinemachineCore.Stage.Finalize:
                {
                    if (NoiseCancellation)
                    {
                        ...
                        {
                            state.RawtOrientation = Quaternion.LookRotation(dir, state.ReferenceUp);
                    ...
                    else
                    {
                        ...
                        if (player != null)
                        {
                            state.ReferenceLookAt = ComputeLookAtPoint(
                                state.GetCorrectedPosition(), player, state.GetCorrectedtOrientation() * Vector3.forward);
                            AimTarget = ComputeAimTarget(state.ReferenceLookAt, player);
                        }
                    }
                    break;
        ...
        Vector3 ComputeLookAtPoint(Vector3 camPos, Transform player, Vector3 fwd)
        {
            ...
            var playertOrientation = player.rotation;
            var playerPosLocal = Quaternion.Inverse(playertOrientation) * (player.position - camPos);
