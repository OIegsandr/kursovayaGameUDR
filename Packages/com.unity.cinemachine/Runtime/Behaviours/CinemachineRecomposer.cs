    ...
    public class CinemachineRecomposer : CinemachineExtension
    {
            ...
            CinemachineCore.Stage stage, ref CameraState state, float deltaTime)
        {
            if (stage == ApplyAfter)
            {
                ...
                // Tilt by local X
                var qTilted = state.RawtOrientation * Quaternion.AngleAxis(Tilt, Vector3.right);
                // Pan in world space
                var qDesired = Quaternion.AngleAxis(Pan, state.ReferenceUp) * qTilted;
                state.tOrientationCorrection = Quaternion.Inverse(state.GetCorrectedtOrientation()) * qDesired;
