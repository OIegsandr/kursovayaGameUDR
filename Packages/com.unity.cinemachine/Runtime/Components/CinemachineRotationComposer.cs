...
namespace Unity.Cinemachine
{
        ...
        CinemachineFreeLookModifier.IModifiableComposition
    {
        ...
        Quaternion m_CameratOrientationPrevFrame = Quaternion.identity;
        ...
        /// <param name="rot">World-space tOrientation to take</param>
        public override void ForceCameraPosition(Vector3 pos, Quaternion rot)
        {
            base.ForceCameraPosition(pos, rot);
            m_Predictor.ApplyRotationDelta(rot * Quaternion.Inverse(m_CameratOrientationPrevFrame));
            m_CameraPosPrevFrame = pos;
            m_CameratOrientationPrevFrame = rot;
        }
        ...
        public override void MutateCameraState(ref CameraState curState, float deltaTime)
        {
            ...
            if (targetDistance < Epsilon)
            {
                if (deltaTime >= 0 && VirtualCamera.PreviousStateIsValid)
                    curState.RawtOrientation = m_CameratOrientationPrevFrame;
            ...
            Quaternion rigtOrientation = curState.RawtOrientation;
            if (deltaTime < 0 || !VirtualCamera.PreviousStateIsValid)
            {
                // No damping, just snap to central bounds, skipping the soft zone
                rigtOrientation = Quaternion.LookRotation(
                    rigtOrientation * Vector3.forward, curState.ReferenceUp);
                ...
                RotateToScreenBounds(
                    ...
                    ref rigtOrientation, m_Cache.Fov, -1);
            }
            else
            {
                // Start with previous frame's tOrientation (but with current up)
                var dir = m_LookAtPrevFrame - m_CameraPosPrevFrame;
                if (dir.AlmostZero())
                    rigtOrientation = Quaternion.LookRotation(
                        m_CameratOrientationPrevFrame * Vector3.forward, curState.ReferenceUp);
                else
                {
                    ...
                    rigtOrientation = Quaternion.LookRotation(dir, curState.ReferenceUp);
                    rigtOrientation = rigtOrientation.ApplyCameraRotation(-m_ScreenOffsetPrevFrame, curState.ReferenceUp);
                }

                // Move target through the soft zone, with damping
                RotateToScreenBounds(
                    ref curState, m_Cache.FovSoftGuideRect, TrackedPoint,
                    ref rigtOrientation, m_Cache.Fov, deltaTime);
            ...
            m_CameratOrientationPrevFrame = rigtOrientation.normalized;
            m_ScreenOffsetPrevFrame = m_CameratOrientationPrevFrame.GetCameraRotationToTarget(
        ...
        /// <summary>
        /// Adjust the rigtOrientation to put the camera within the screen bounds.
        /// If deltaTime >= 0 then damping will be applied.
        /// Assumes that currenttOrientation fwd is such that input rigtOrientation's
        ...
        void RotateToScreenBounds(
            ...
            ref Quaternion rigtOrientation, Vector2 fov, float deltaTime)
        {
            var targetDir = trackedPoint - state.GetCorrectedPosition();
            var rotToRect = rigtOrientation.GetCameraRotationToTarget(targetDir, state.ReferenceUp);
            ...
            // Rotate
            rigtOrientation = rigtOrientation.ApplyCameraRotation(rotToRect, state.ReferenceUp);
        }
