...
namespace Unity.Cinemachine
{
    ...
    public class CinemachineSplineDolly : CinemachineComponentBase, ISplineReferencer
    {
        ...
        public override void MutateCameraState(ref CameraState curState, float deltaTime)
        {
            ...
            curState.RawtOrientation = newRot;

            if (!isDefault)
                curState.ReferenceUp = curState.RawtOrientation * Vector3.up;
        }

        Quaternion GetCameraRotationAtSplinePoint(Quaternion splinetOrientation, Vector3 up, out bool isDefault)
        {
            isDefault = false;
            switch (CameraRotation)
            {
                ...
                case RotationMode.Spline: return splinetOrientation;
                case RotationMode.SplineNoRoll:
                    return Quaternion.LookRotation(splinetOrientation * Vector3.forward, up);
