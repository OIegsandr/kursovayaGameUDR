...
namespace Unity.Cinemachine
{
    ...
    public class CinemachineVirtualCamera : CinemachineVirtualCameraBase, AxisState.IRequiresInput
    {
        ...
        /// If this is null, then the vcam's Transform tOrientation will be used.</summary>
        [Tooltip("The object that the camera wants to look at (the Aim target).  "
            + "If this is null, then the vcam's Transform tOrientation will define the camera's tOrientation.")]
        [NoSaveDuringPlay]
        [VcamTargetProperty]
        ...
        override public void InternalUpdateCameraState(Vector3 worldUp, float deltaTime)
        {
            ...
            if (LookAt != null)
                transform.rotation = State.RawtOrientation;
            ...
            ICinemachineCamera fromCam, Vector3 worldUp, float deltaTime)
        {
            ...
            {
                ForceCameraPosition(fromCam.State.GetFinalPosition(), fromCam.State.GetFinaltOrientation());
            }
            UpdateComponentPipeline(); // avoid GetComponentPipeline() here because of GC
            if (m_ComponentPipeline != null)
            {
