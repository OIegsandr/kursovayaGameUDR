    ...
    public class CinemachineTrackedDolly : CinemachineComponentBase
    {
        ...
        public override void MutateCameraState(ref CameraState curState, float deltaTime)
        {
            ...
            if (deltaTime < 0 || !VirtualCamera.PreviousStateIsValid)
            {
                ...
                m_PrevioustOrientation = curState.RawtOrientation;
            }
            ...
            Quaternion newPathtOrientation = m_Path.EvaluatetOrientationAtUnit(newPathPosition, m_PositionUnits);
            ...
            Vector3 offsetX = newPathtOrientation * Vector3.right;
            Vector3 offsetY = newPathtOrientation * Vector3.up;
            Vector3 offsetZ = newPathtOrientation * Vector3.forward;
            ...
            // Set the tOrientation and up
            Quaternion newtOrientation
                = GetCameratOrientationAtPathPoint(newPathtOrientation, curState.ReferenceUp);
            if (deltaTime >= 0 && VirtualCamera.PreviousStateIsValid)
            {
                Vector3 relative = (Quaternion.Inverse(m_PrevioustOrientation)
                    * newtOrientation).eulerAngles;
                for (int i = 0; i < 3; ++i)
                    if (relative[i] > 180)
                        relative[i] -= 360;
                relative = Damper.Damp(relative, AngularDamping, deltaTime);
                newtOrientation = m_PrevioustOrientation * Quaternion.Euler(relative);
            }
            m_PrevioustOrientation = newtOrientation;

            curState.RawtOrientation = newtOrientation;
            if (m_CameraUp != CameraUpMode.Default)
                curState.ReferenceUp = curState.RawtOrientation * Vector3.up;
        }

        private Quaternion GetCameratOrientationAtPathPoint(Quaternion pathtOrientation, Vector3 up)
        {
            switch (m_CameraUp)
            {
                default:
                case CameraUpMode.Default: break;
                case CameraUpMode.Path: return pathtOrientation;
                case CameraUpMode.PathNoRoll:
                    return Quaternion.LookRotation(pathtOrientation * Vector3.forward, up);
        ...
        Quaternion m_PrevioustOrientation = Quaternion.identity;
