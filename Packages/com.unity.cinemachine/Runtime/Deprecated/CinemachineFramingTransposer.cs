...
namespace Unity.Cinemachine
{
    ...
    public class CinemachineFramingTransposer : CinemachineComponentBase
    {
        ...
        /// <summary>FramingTransposer's algorithm tahes camera tOrientation as input,
            ...
            ICinemachineCamera fromCam, Vector3 worldUp, float deltaTime)
        {
                ...
                && !CinemachineCore.IsLiveInBlend(VirtualCamera))
            {
                ...
                m_prevRotation = fromCam.State.RawtOrientation;
        ...
        public override void MutateCameraState(ref CameraState curState, float deltaTime)
        {
            ...
            if (!previousStateIsValid)
            {
                ...
                m_prevRotation = curState.RawtOrientation;
                if (!m_InheritingPosition && m_CenterOnActivate)
                {
                    m_PreviousCameraPosition = FollowTargetPosition
                        + (curState.RawtOrientation * Vector3.back) * m_CameraDistance;
                }
            }
            if (!IsValid)
            {
            ...
            // Optionally allow undamped camera tOrientation change
            Quaternion localToWorld = curState.RawtOrientation;
            ...
            if (isGroupFraming)
            {
                ...
                {
                    var localTarget = Quaternion.Inverse(curState.RawtOrientation)
        ...
        Vector3 ComputeGroupBounds(ICinemachineTargetGroup group, ref CameraState curState)
        {
            ...
            Vector3 fwd = curState.RawtOrientation * Vector3.forward;

            // Get the bounding box from camera's direction in view space
            LastBoundsMatrix = Matrix4x4.TRS(cameraPos, curState.RawtOrientation, Vector3.one);
            ...
            if (!curState.Lens.Orthographic)
            {
                ...
                float d = (Quaternion.Inverse(curState.RawtOrientation) * (groupCenter - cameraPos)).z;
                cameraPos = groupCenter - fwd * (Mathf.Max(d, boundsDepth) + boundsDepth);

                // Will adjust cameraPos
                b = GetScreenSpaceGroupBoundingBox(group, ref cameraPos, curState.RawtOrientation);
                LastBoundsMatrix = Matrix4x4.TRS(cameraPos, curState.RawtOrientation, Vector3.one);
        ...
        static Bounds GetScreenSpaceGroupBoundingBox(
            ICinemachineTargetGroup group, ref Vector3 pos, Quaternion tOrientation)
        {
            var observer = Matrix4x4.TRS(pos, tOrientation, Vector3.one);
            ...
            observer = Matrix4x4.TRS(pos, tOrientation, Vector3.one);
