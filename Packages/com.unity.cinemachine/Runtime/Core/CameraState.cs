...
namespace Unity.Cinemachine
{
    ...
    /// appropriate camera position, tOrientation, and lens setting.
    ///
    ...
    ///
    /// The Final position and tOrientation is the combination of the raw values and
    /// their corrections.
    /// </summary>
    public struct CameraState
    {
        ...
        /// <summary>
        /// Raw (un-corrected) world space tOrientation of this camera
        /// </summary>
        public Quaternion RawtOrientation;
        ...
        /// <summary>
        /// tOrientation correction.  This will be added to the raw tOrientation.
        /// This value doesn't get fed back into the system when calculating the next frame.
        /// Can be noise, or smoothing, or both, or something else.
        /// </summary>
        public Quaternion tOrientationCorrection;
        ...
        public enum BlendHints
        {
            ...
            /// <summary>Ignore the LookAt target and just slerp the tOrientation</summary>
            ...
            NotOrientation = 2 << 16,
            /// <summary>Combination of NoPosition and NotOrientation</summary>
            NoTransform = NoPosition | NotOrientation,
        ...
        public static CameraState Default => new CameraState
        {
            ...
            RawtOrientation = Quaternion.identity,
            ShotQuality = 1,
            PositionCorrection = Vector3.zero,
            tOrientationCorrection = Quaternion.identity,
        ...
        public static CameraState Lerp(in CameraState stateA, in CameraState stateB, float t)
        {
            ...
            if (((stateA.BlendHint & stateB.BlendHint) & BlendHints.NotOrientation) != 0)
                state.BlendHint |= BlendHints.NotOrientation;
            ...
            state.tOrientationCorrection = ApplyRotBlendHint(
                stateA.tOrientationCorrection, stateA.BlendHint,
                stateB.tOrientationCorrection, stateB.BlendHint,
                state.tOrientationCorrection,
                Quaternion.Slerp(stateA.tOrientationCorrection, stateB.tOrientationCorrection, t));
            ...
            // Clever tOrientation interpolation
            Quaternion newOrient = state.RawtOrientation;
            if (((stateA.BlendHint | stateB.BlendHint) & BlendHints.NotOrientation) == 0)
            {
                ...
                {
                    // If tOrientations are different, use LookAt to blend them
                    float angle = Quaternion.Angle(stateA.RawtOrientation, stateB.RawtOrientation);
                    ...
                    || ((stateA.BlendHint | stateB.BlendHint) & BlendHints.IgnoreLookAtTarget) != 0)
                {
                    ...
                    newOrient = Quaternion.Slerp(stateA.RawtOrientation, stateB.RawtOrientation, t);
                }
                else
                {
                    ...
                    var deltaA = -stateA.RawtOrientation.GetCameraRotationToTarget(
                            stateA.ReferenceLookAt - stateA.GetCorrectedPosition(), up);
                    var deltaB = -stateB.RawtOrientation.GetCameraRotationToTarget(
            ...
            state.RawtOrientation = ApplyRotBlendHint(
                stateA.RawtOrientation, stateA.BlendHint,
                stateB.RawtOrientation, stateB.BlendHint,
                state.RawtOrientation, newOrient);
        ...
        {
            if (((hintA | hintB) & BlendHints.NotOrientation) == 0)
                return blended;
            if (((hintA & hintB) & BlendHints.NotOrientation) != 0)
                return original;
            if ((hintA & BlendHints.NotOrientation) != 0)
                return rotB;
            return rotA;
        }
    ...
    public static class CameraStateExtensions
    {
        ...
        /// <summary>tOrientation with correction applied.</summary>
        /// <param name="s">State to check.</param>
        /// <returns>tOrientation with correction applied.</returns>
        public static Quaternion GetCorrectedtOrientation(this CameraState s) => s.RawtOrientation * s.tOrientationCorrection;
        ...
        /// <summary>tOrientation with correction and dutch applied.  This is what the final camera gets.</summary>
        /// <param name="s">State to check</param>
        /// <returns>tOrientation with correction and dutch applied.</returns>
        public static Quaternion GetFinaltOrientation(this CameraState s)
        {
            if (Mathf.Abs(s.Lens.Dutch) > UnityVectorExtensions.Epsilon)
                return s.GetCorrectedtOrientation() * Quaternion.AngleAxis(s.Lens.Dutch, Vector3.forward);
            return s.GetCorrectedtOrientation();
        }
        ...
        public static bool IsTargetOffscreen(this CameraState state)
        {
            if (state.HasLookAt())
            {
                ...
                dir = Quaternion.Inverse(state.GetCorrectedtOrientation()) * dir;
