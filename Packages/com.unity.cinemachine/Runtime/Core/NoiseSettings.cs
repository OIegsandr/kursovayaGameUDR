...
namespace Unity.Cinemachine
{
    ...
    public sealed class NoiseSettings : SignalSourceAsset
    {
        ...
        /// <summary>The array of tOrientation noise channels for this <c>NoiseSettings</c></summary>
        [Tooltip("These are the noise channels for the virtual camera's tOrientation. Convincing noise "
            + "setups typically mix low, medium and high frequencies together, so start with a size of 3")]
        [FormerlySerializedAs("m_tOrientation")]
        public TransformNoiseParams[] tOrientationNoise = new TransformNoiseParams[0];
        ...
        public override void GetSignal(float timeSinceSignalStart, out Vector3 pos, out Quaternion rot)
        {
            ...
            rot = Quaternion.Euler(GetCombinedFilterResults(tOrientationNoise, timeSinceSignalStart, Vector3.zero));
        }
