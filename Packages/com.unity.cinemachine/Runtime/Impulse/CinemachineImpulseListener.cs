    ...
    public class CinemachineImpulseListener : CinemachineExtension
    {
           ...
           public float FrequencyGain;
                ...
                out Vector3 pos, out Quaternion rot)
            {
                ...
                rot = Quaternion.Euler(NoiseSettings.GetCombinedFilterResults(
                        m_SecondaryNoise.tOrientationNoise, m_CurrentTime, m_NoiseOffsets) * gain);
            ...
            CinemachineCore.Stage stage, ref CameraState state, float deltaTime)
        {
            if (stage == ApplyAfter && deltaTime >= 0)
            {
                ...
                {
                    if (UseCameraSpace)
                        impulsePos = state.RawtOrientation * impulsePos;
                    state.PositionCorrection += impulsePos;
                    state.tOrientationCorrection = state.tOrientationCorrection * impulseRot;
                }
            }
        }
    }
